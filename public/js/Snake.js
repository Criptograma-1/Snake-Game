export default class Snake {
  constructor(scene) {
    ...
    this.tileSize = 16;
    this.direction = Phaser.Math.Vector2.LEFT; //inicial direction
    this.body = [];
    this.body.push(
      this.scene.add
        .rectangle(
  ...
  positionApple() {
    //randomly positioning the apple
    this.apple.x = 
      Math.floor(
  ...
  move() {
    //make the head fit the grid
    let x = this.body[0].x + this.direction.x * this.tileSize;
    let y = this.body[0].y + this.direction.y * this.tileSize;

    if(this.apple.x === x && this.apple.y === y) {
      //eat the apple extends the snake's body
      this.body.push(
        this.scene.add
          .rectangle(0,0,this.tileSize,this.tileSize,0xffffff)
          .setOrigin(0)
      );
      this.positionApple(); //reposition the apple
    }

    for (let index = this.body.length - 1; index > 0; index--){
      //make the body move with the head
      this.body[index].x = this.body[index-1].x;
      this.body[index].y = this.body[index-1].y;
    }
    //moves the head in the horizontal
    this.body[0].x = x;
    //moves the head in the vertical
    this.body[0].y = y;
